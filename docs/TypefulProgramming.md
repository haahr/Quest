...
- [2. Typeful languages](#2-typeful-languages)
  - [2.1. Relevant concepts](#21-relevant-concepts)
  - [2.2. Theory and practice](#22-theory-and-practice)
  - [2.3. Why types?](#23-why-types)
  - [2.4. Why subtypes?](#24-why-subtypes)
  - [2.5. Why polymorphism?](#25-why-polymorphism)
- [3. Quantifiers and subtypes](#3-quantifiers-and-subtypes)
  - [3.1. Kinds, types, and values](#31-kinds-types-and-values)
  - [3.2. Signatures and bindings](#32-signatures-and-bindings)
  - [3.3. The Quest language](#33-the-quest-language)
- [4. The kind of types](#4-the-kind-of-types)
  - [4.1. Introduction](#41-introduction)
  - [4.2. Basic and built-in types](#42-basic-and-built-in-types)
  - [4.3. Function types](#43-function-types)
  - [4.4. Tuple types](#44-tuple-types)
  - [4.5. Option types](#45-option-types)
  - [4.6. Auto types](#46-auto-types)
  - [4.7. Recursive types](#47-recursive-types)
  - [4.8. Mutable types](#48-mutable-types)
  - [4.9. Exception types](#49-exception-types)
- [5. Operator kinds](#5-operator-kinds)
  - [5.1. Type operators](#51-type-operators)
  - [5.2. Recursive type operators](#52-recursive-type-operators)
- [6. Power kinds](#6-power-kinds)
  - [6.1. Tuple subtypes](#61-tuple-subtypes)
  - [6.2. Option subtypes](#62-option-subtypes)
  - [6.3. Record and variant types](#63-record-and-variant-types)
  - [6.4. Higher-order subtypes](#64-higher-order-subtypes)
  - [6.5. Bounded universal quantifiers](#65-bounded-universal-quantifiers)
  - [6.6. Bounded existential quantifiers](#66-bounded-existential-quantifiers)
  - [6.7. Auto subtypes](#67-auto-subtypes)
  - [6.8. Mutable subtypes](#68-mutable-subtypes)
  - [6.9. Recursive subtypes](#69-recursive-subtypes)
- [7. Large programs](#7-large-programs)
  - [7.1. Interfaces and modules](#71-interfaces-and-modules)
  - [7.2. Manifest types and kinds](#72-manifest-types-and-kinds)
  - [7.3. Diamond import](#73-diamond-import)
- [8. Huge programs](#8-huge-programs)
  - [8.1. Open systems](#81-open-systems)
  - [8.2. Closed systems](#82-closed-systems)
  - [8.3. Sealed systems](#83-sealed-systems)
- [9. System programs](#9-system-programs)
  - [9.1. Dynamic types](#91-dynamic-types)
  - [9.2. Stack allocation](#92-stack-allocation)
  - [9.3. Type violations](#93-type-violations)
- [10. Conclusions](#10-conclusions)
  - [10.1. This style](#101-this-style)
  - [10.2. Other styles](#102-other-styles)
  - [10.3. Acknowledgments](#103-acknowledgments)
- [11. Appendix](#11-appendix)
  - [11.1. Syntax](#111-syntax)
  - [11.2. Type rules](#112-type-rules)
  - [11.3. Library interfaces](#113-library-interfaces)
...
| tuple | `tuple` $l_1=a_1 .. l_n=a_n$ `end` | `b.`$l$ |
...
+--------|---------------------|--------------------|-----|---+
| Level 1|                     |                    |     |   |
|   Int  |             Fun(A::TYPE)All(x:A)A        |     |   |
|   All(A::TYPE)All(x:A)A                           A  <: B   |
|   Pair A::TYPE x:A end                            ^     ^   |
|        ^                                          |     |   |
+--------|------------------------------------------|-----|---+
| values |                                          |     |   |
...
Types in the Quest language have a much richer structure, including type variables, type operators, and a notion of type computation. In fact, the Quest type level is itself a very expressive $\lambda$-calculus, as opposed to a simple algebra of type operators. Moreover, this $\lambda$-calculus is "typed", where the "types" here are the kinds, since we are one level "higher" than normal. This should become clearer by the end of this section.
...
- `All(x:A)B`  
  This is the type of functions from values in `A` to values in `B`, where `A` and `B` are types. The variable `x` can appear in `B` only in special circumstances, so this is normally equivalent to the function space `A->B`.  
  Sample element: `fun(x:Int)x`.

- `All(X::K)B`  
  This is the type of functions from types in `K` to values in `B`, where `K` is a kind, `B` is a type, and `X` may occur in `B`.  
  Sample element: `fun(A::TYPE)fun(x:A)x`.
...
- `Pair x:A y:B end`  
  ...
  Sample element: `pair let x=true let y=3 end`.
...
| Abbreviation | For |
| :--- | :--- |
| $x_1, \dots, x_n : B$ | $x_1:B \dots x_n:B$ |
| $X_1, \dots, X_n :: L$ | $X_1::L \dots X_n::L$ |
| $x(S_1)\dots(S_n) : B$ | $x : 	ext{All}(S_1)\dots	ext{All}(S_n) B$ |
| $X(S_1)\dots(S_n) :: L$ | $X :: 	ext{ALL}(S_1)\dots	ext{ALL}(S_n) L$ |

**Bindings:**

| Abbreviation | For |
| :--- | :--- |
| `let` $x(S_1)\dots(S_n) : B = b$ | `let` $x = 	ext{fun}(S_1)\dots	ext{fun}(S_n) : B \; b$ |
| `Let` $X(S_1)\dots(S_n) :: L = B$ | `Let` $X = 	ext{Fun}(S_1)\dots	ext{Fun}(S_n) :: L \; B$ |

Signatures and bindings can be used in many different places in a language, and hence make the syntax more uniform. The positions where signatures and bindings appear in a surrounding context are shown below; note the use of some of the abbreviations just introduced:
...
The design principles and ideas we have discussed so far, as well as some yet to be discussed, are incorporated in a programming language called **Quest** (a quasi-acronym for *Qu*antifiers and *S*ub*t*ypes). This language illustrates fundamental concepts of typeful programming, and how they can be concretely embedded and integrated in a single language. The language will be described in detail in the following sections, where it will provide a basis for further discussions and examples.
...
Quest is used throughout the rest of the paper. The three major sections to follow deal with:
- a) the kind of types, including basic types, various structured types, polymorphism, abstract types, recursive types, dynamic types, mutable types and exception types;
- b) the kinds of operators, including parametric, recursive, and higher-order type operators; and
- c) the kinds of subtypes, including inheritance and bounded quantification.

Then follow two sections on programming with modules and interfaces, seen as values and types respectively, and a section on system programming from the point of view of typing violations.
...
- There are user-definable higher-order type operators and computations at the type level. The typechecker includes a normal-order typed $\lambda$-calculus evaluator (where the "types" here are actually the kinds).
...
- New infix operators can be declared freely (e.g. `let ++(x,y:Int):Int = 2*{x+y}`). There is no declaration of infix status. (It is not needed because of the strict lexical conventions.) There is no declaration of infix precedence. All infix operators, including the built-in ones, have the same precedence and are right-associative. This decision has been made because it is difficult to specify the relative precedence of operators in a meaningful way, and because it is equally difficult to remember or guess the relative precedence of operators when new infixes can be introduced.
...
| Construct | Same as |
| :--- | :--- |
| `a andif b` | `if a then b else false end` |
| `a orif b` | `if a then true else b end` |
...
` is newline, `	` is tab, `` is backspace, `` is form feed, `"` is double quote, and `\` is backslash; backslash followed by any other character is equivalent to that character.
...
Since the choices in an option type are ordered, there is an operator `ordinal(o)` returning the 0-based ordinal number of an option as determined by its tag[^3]. Note that the tag component of an option can never be modified.

[^3]: Conversely, an option can be created from an ordinal number as `option ordinal(n) of T .. end`, where `n` is determined at run-time; this form is allowed only when all the branches of an option type have the same signature.
...
| Form | Equivalent to |
| :--- | :--- |
| `when x,y with z then a` | `when x with z then a`<br>`when y with z then a` |
...
```quest
Option                             Option
  with x:Bool end                    a with x,y:Bool end
  a with y:Bool end                  b with x,z:Bool end
  b with z:Bool end                end;
end;
```
...
The usual Lisp primitives[^4] can now be defined in terms of operations on options and tuples; this is left as an exercise.
...
Imperative programming is based on the notion of a mutable global store, together with constructs for sequencing the operations affecting the store. Mutability interacts very nicely with all the quantifiers, including polymorphism[^5], showing that the functional-style approach suggested by type theory does not prevent the design of imperative languages.

[^5]: The problems encountered in ML [Gordon Milner Wadsworth 79, page 52] are avoided by the use of explicit polymorphism. However, we have to take some precautions with subtyping, as explained later.
...
| Form | For |
| :--- | :--- |
| `f of .. end` | `f(array of .. end)` |
| `f of(n a)` | `f(array of(n a))` |
...
The `exception` construct generates a new unique exception value whenever it is evaluated. The identifier in it is used only for printing exception messages; the type is the type of a value that can accompany the exception.
...
We can then talk of the collection of all subtypes of a given type $B$; this forms a kind which we call `POWER(B)`, the power-kind of $B$. Then `A::POWER(B)` and `A<:B` have the same meaning; the former is taken as the actual primitive, while the latter is considered an abbreviation. Subtyping induces a subkind relation, written `<::`, on kinds; basically, `POWER(A)<::POWER(B)` whenever `A<:B`, and `POWER(A)<::TYPE` for any type $A$. Then we say that a signature $S$ is a *subsignature* of a signature $S'$ if $S$ has the same number of components as $S'$ with the same names and in the same position, and if the component types (or kinds) of $S$ are subtypes (or subkinds) of the corresponding components in $S'$. Moreover, we say that $S''$ is an *extended subsignature* of $S'$ if $S''$ has a prefix $S$ which is a subsignature of $S'$. The formal subtyping rules for a simplified Quest language are in the Appendix.
...
Records are named collections of values, where all the names are distinct. Unlike tuples, record components are unordered, they must always be named, and they cannot introduce types[^6].

```quest
Record x1:A1 .. xn:An end
```

This is the type of a record containing (a permutation of) the required named components:

```quest
record x1:A1=a1 .. xn:An=an end
```
...
Otherwise, variants and variant types are similar to options and option types, along with the `?` and `!` operators and the `case` construct[^7].
...
By adopting this rule for functions, we take "ground" subtyping, at the data-structure level, and "lift" it to higher function spaces. Since object-oriented programming is based on ground subtyping, and functional programming is based on higher-order functions, we can say that the rule above unifies functional and object-oriented programming, at least at the type level.
...
Here we have unfortunately forgotten that `myCar` was a `Car`, just by passing it through an identity function. This problem can be solved by providing more type information:
...
$$A \cap B <: A$$
$$A \cap B <: B$$
$$A <: B \wedge A <: C \implies A <: B \cap C$$
...
$$A <: C \implies A \cap B <: C$$
$$A <: B \cap C \implies A <: B$$
$$A <: B \wedge C <: D \implies A \cap C <: B \cap D$$

The conversions between $A$ and $	ext{In}(A)$ are rather trivial; they are performed by `let` variable declarations ($A 	o 	ext{In}(A)$) and by variable access ($	ext{In}(A) 	o A$), with subtyping rule $A<:B \implies 	ext{In}(A)<:	ext{In}(B)$. For simplicity, in Quest we identify the types $A$ and $	ext{In}(A)$, and take their conversions as implicit. This amounts to assuming $	ext{In}(A)<:A$ and $A<:	ext{In}(A)$.
...
$$(1) \quad A <: B \implies 	ext{Out}(B) <: 	ext{Out}(A)$$
...
$$(2') \quad 	ext{Var}(A) <: A$$
$$(3') \quad 	ext{Var}(A) <: 	ext{Out}(A)$$

with the more general forms:

$$(2) \quad A <: B \implies 	ext{Var}(A) <: B$$
$$(3) \quad B <: A \implies 	ext{Var}(A) <: 	ext{Out}(B)$$

and, putting the last two together:

$$(4) \quad (A <: B) \wedge (B <: A) \implies 	ext{Var}(A) <: 	ext{Var}(B)$$
...
Since $	ext{Var}(A)<:	ext{Out}(A)$, it becomes possible to pass a mutable location as an `out` parameter, but the explicit conversions `@` and `var` are required to prevent the automatic fetching of mutable locations described above[^8]:
...
In-out `var` parameters could be used in addition to `out` parameters, but because of (4) they would be less flexible from a subtyping point of view. For simplicity, they are left out of the language[^9].

[^9]: Note: the implicit conversion `Var(A)<:A` is problematic for a natural implementation of in-out parameters as pointer-displacement pairs.

The following example shows that relaxing rule (4) to $A<:B \implies 	ext{Var}(A)<:	ext{Var}(B)$ is semantically unsound:
...
$$	ext{Array}(A) \quad 	ext{regarded as} \quad 	ext{All}(:	ext{Int}) \; 	ext{Tuple} : 	ext{Var}(A) \; 	ext{end}$$

Hence we obtain the rule:

$$(5) \quad (A <: B) \wedge (B <: A) \implies 	ext{Array}(A) <: 	ext{Array}(B)$$
...
Modules are *linked* by importing them at the top level[^10].
...
[^10]: In view of dynamic linking, it might be nicer to introduce `import` as a new form of binding that would not be restricted to the top level.
...
• interface C                      • module c:C
    import a:A                         import a:A
    export                             export
      f(x:a.T):Int                       let f(x:a.T):Int = a.int(x)+1
  end;                               end;
...
```quest
• system U                   • system S of U             • system T of U
  end;                         end;                        end;

• interface A of S           • interface B of S          • interface C of S
...
The solution now is to add a new interface `J` exporting a real abstract type `T` and its operations; the implementation of `J` realizes `T` by `I_T`, imported from `I`, and implements the operations by importing them from `m1` and `m2`. Then everything is wrapped into a sealed system that exports only `J`.
...
• interface I1 of W                • module m1:I1
    import I                           import I
    export                             export
      f: ...                             let f = ...
  end;                               end;

• interface I2 of W                • module m2:I2
    import I                           import I
    export                             export
      g: ...                             let g = ...
  end;                               end;

• interface J of W                 • module n:J
...
We adopt essentially the same scheme here, except we use the keyword `unsound`[^13]. Unsound modules may advertise an unsound interface. However, unsound modules can also have ordinary interfaces. In the latter case, the programmer of the unsound module guarantees (i.e. proves or, more likely, trusts) that the module is not actually unsound, from an external point of view, although internally it uses unsound operations. This way, low-level facilities can be added to the system without requiring all the users of such facilities to declare their modules unsound just because they import them.

[^13]: Soundness is the property one proves to show that a type system does not permit type violations. The word *unsafe* has a slightly different meaning in the languages above, since some unsound operations are considered safe if they do not violate run-time invariants.
...
$$	ext{All}(S)A$$
$$	ext{Tuple}(S)$$
$$	ext{Fun}(X::K)A \qquad A(B)$$
$$	ext{Rec}(X::	ext{TYPE})A$$

**Values ($a, b, c$; value identifiers $x, y, z$):**
$$x$$
$$	ext{fun}(S)a \qquad a(D)$$
$$	ext{tuple}(D) \qquad 	ext{bind } S = a 	ext{ in } b$$
$$	ext{rec}(x:A)a$$

Here we use the construct $	ext{bind } S = a 	ext{ in } b$ which binds the components of the tuple $a$ to the identifiers in $S$ in the scope $b$. In full Quest we use instead the notation $x.Y$ and $x.y$ to extract types and values out of a tuple denoted by an identifier $x$. Then a program fragment $	ext{let } x = a \dots x.Y \dots x.y \dots$ corresponds to $	ext{bind } \dots Y::K \dots y:A \dots = a 	ext{ in } \dots y \dots Y \dots$. The type rule for $	ext{bind}$ prevents the type identifiers in $S$ from occurring in the type of the result. Similarly, in full Quest types like $x.Y$ are not allowed to escape the scope of $x$. The formal relation between $	ext{bind}$ and $x.Y$/$x.y$ is studied in [Cardelli Leroy 90].

#### Judgments

| Judgment | Meaning |
| :--- | :--- |
| $dash S 	ext{ sig}$ | $S$ is a signature |
| $S dash K 	ext{ kind}$ | $K$ is a kind |
| $S dash A 	ext{ type}$ | $A$ is a type (same as $S dash A :: 	ext{TYPE}$) |
| $S dash D 	herefore S'$ | $D$ has signature $S'$ |
| $S dash A :: K$ | $A$ has kind $K$ |
| $S dash a : A$ | $a$ has type $A$ |
| $S dash S' <	herefore S''$ | $S'$ is a subsignature of $S''$ |
| $S dash K <:: L$ | $K$ is a subkind of $L$ |
| $S dash A <: B$ | $A$ is a subtype of $B$ (same as $S dash A :: 	ext{POWER}(B)$) |
| $S dash S' <	herefore> S''$ | equivalent signatures |
| $S dash K <::> L$ | equivalent kinds |
| $S dash A <:> B$ | equivalent types |
...
- $E\{X \leftarrow A\}$ and $E\{x \leftarrow a\}$ denote the substitution of the type variable $X$ by the type $A$, or of the variable $x$ by the value $a$, within an expression $E$ of any sort. For a binding $D$, $E\{D\}$ is defined as follows:
  - $E\{\emptyset\} = E$
  - $E\{D', X::K=A\} = E\{X \leftarrow A\}\{D'\}$
  - $E\{D', x:A=a\} = E\{x \leftarrow a\}\{D'\}$
...
$$rac{S dash A :: K}{S dash A <:> A} \qquad rac{S dash K 	ext{ kind}}{S dash K <::> K} \qquad rac{dash S \, S' 	ext{ sig}}{S dash S' <	herefore> S'}$$

$$rac{S dash A <:> A'}{S dash A' <:> A} \qquad rac{S dash K <::> K'}{S dash K' <::> K} \qquad rac{S dash S' <	herefore> S''}{S dash S'' <	herefore> S'}$$

$$rac{S dash A <:> A' \quad S dash A' <:> A''}{S dash A <:> A''} \qquad rac{S dash K <::> K' \quad S dash K' <::> K''}{S dash K <::> K''} \qquad rac{S dash S' <	herefore> S'' \quad S dash S'' <	herefore> S'''}{S dash S' <	herefore> S'''}$$

#### Congruence

$$rac{S dash A[B] :: K \quad S dash B <:> B'}{S dash A[B] <:> A[B']} \qquad rac{S dash A[K] :: L \quad S dash K <::> K'}{S dash A[K] <:> A[K']} \qquad rac{S dash A[S'] :: L \quad S dash S' <	herefore> S''}{S dash A[S'] <:> A[S'']}$$

$$rac{S dash K[A] 	ext{ kind} \quad S dash A <:> A'}{S dash K[A] <::> K[A']} \qquad rac{S dash K[L] 	ext{ kind} \quad S dash L <::> L'}{S dash K[L] <::> K[L']} \qquad rac{S dash K[S'] 	ext{ kind} \quad S dash S' <	herefore> S''}{S dash K[S'] <::> K[S'']}$$

$$rac{dash S \, S' 	ext{ sig} \quad S dash A <:> A'}{S dash S'[A] <	herefore> S'[A']} \qquad rac{dash S \, S'[K] 	ext{ sig} \quad S dash K <::> K'}{S dash S'[K] <	herefore> S'[K']} \qquad rac{dash S \, S'[S''] 	ext{ sig} \quad S dash S'' <	herefore> S'''}{S dash S'[S''] <	herefore> S'[S''']}$$

#### Inclusion

$$rac{S dash A <:> B}{S dash A <: B} \qquad rac{S dash K <::> L}{S dash K <:: L} \qquad rac{S dash S' <	herefore> S''}{S dash S' <	herefore S''}$$

#### Subsumption

$$rac{S dash a : A \quad S dash A <: B}{S dash a : B} \qquad rac{S dash A :: K \quad S dash K <:: L}{S dash A :: L} \qquad rac{S dash D 	herefore S' \quad S dash S' <	herefore S''}{S dash D 	herefore S''}$$

#### Conversion

$$rac{S, X::K dash B :: L \quad S dash A :: K}{S dash (	ext{Fun}(X::K)B)(A) <:> B\{X \leftarrow A\}}$$

$$rac{S dash 	ext{Rec}(X::	ext{TYPE})A 	ext{ type}}{S dash 	ext{Rec}(X::	ext{TYPE})A <:> A\{X \leftarrow 	ext{Rec}(X::	ext{TYPE})A\}}$$

$$rac{S dash A <:> C\{X \leftarrow A\} \quad S dash B <:> C\{X \leftarrow B\} \quad C \downarrow X}{S dash A <:> B}$$

#### Signatures

$$dash \emptyset 	ext{ sig} \qquad rac{S dash K 	ext{ kind} \quad X 
otin 	ext{dom}(S)}{dash S, X::K 	ext{ sig}} \qquad rac{S dash A 	ext{ type} \quad x 
otin 	ext{dom}(S)}{dash S, x:A 	ext{ sig}}$$

#### Bindings

$$rac{dash S 	ext{ sig}}{S dash \emptyset 	herefore \emptyset} \qquad rac{S dash D 	herefore S' \quad S dash A\{D\} :: K\{D\}}{S dash D, X::K = A 	herefore S', X::K} \qquad rac{S dash D 	herefore S' \quad S dash a\{D\} : A\{D\}}{S dash D, x:A = a 	herefore S', x:A}$$

#### Kinds

$$rac{dash S 	ext{ sig}}{S dash 	ext{TYPE kind}} \qquad rac{S dash K 	ext{ kind} \quad S, X::K dash L 	ext{ kind}}{S dash 	ext{ALL}(X::K)L 	ext{ kind}} \qquad rac{S dash A 	ext{ type}}{S dash 	ext{POWER}(A) 	ext{ kind}}$$

#### Types and Operators

$$rac{dash S, X::K, S' 	ext{ sig}}{S, X::K, S' dash X :: K} \qquad rac{S \, S' dash A 	ext{ type}}{S dash 	ext{All}(S')A 	ext{ type}} \qquad rac{dash S \, S' 	ext{ sig}}{S dash 	ext{Tuple}(S') 	ext{ type}}$$

$$rac{S, X::K dash B :: L}{S dash 	ext{Fun}(X::K)B :: 	ext{ALL}(X::K)L} \qquad rac{S dash B :: 	ext{ALL}(X::K)L \quad S dash A :: K}{S dash B(A) :: L\{X \leftarrow A\}} \qquad rac{S, X::	ext{TYPE} dash A 	ext{ type} \quad A \downarrow X}{S dash 	ext{Rec}(X::	ext{TYPE})A 	ext{ type}}$$

#### Values

$$rac{dash S, x:A, S' 	ext{ sig}}{S, x:A, S' dash x : A} \qquad rac{S \, S' dash a : A}{S dash 	ext{fun}(S')a : 	ext{All}(S')A} \qquad rac{S dash a : 	ext{All}(S')A \quad S dash D 	herefore S'}{S dash a(D) : A\{D\}}$$

$$rac{S dash D 	herefore S'}{S dash 	ext{tuple}(D) : 	ext{Tuple}(S')} \qquad rac{S dash a : 	ext{Tuple}(S') \quad S dash B 	ext{ type} \quad S \, S' dash b : B}{S dash 	ext{bind } S' = a 	ext{ in } b : B} \qquad rac{S, x:A dash a : A}{S dash 	ext{rec}(x:A)a : A}$$

#### SubSignatures

$$rac{S dash S' <	herefore S'' \quad S \, S' dash K <:: L}{S dash S', X::K <	herefore S'', X::L} \qquad rac{S dash S' <	herefore S'' \quad S \, S' dash A <: B}{S dash S', x:A <	herefore S'', x:B}$$

#### SubKinds

$$rac{S dash K' <:: K \quad S, X::K' dash L <:: L'}{S dash 	ext{ALL}(X::K)L <:: 	ext{ALL}(X::K')L'} \qquad rac{S dash A 	ext{ type}}{S dash 	ext{POWER}(A) <:: 	ext{TYPE}} \qquad rac{S dash A <: B}{S dash 	ext{POWER}(A) <:: 	ext{POWER}(B)}$$

#### SubTypes

$$rac{S dash S'' <	herefore S' \quad S \, S'' dash A' <: A''}{S dash 	ext{All}(S')A' <: 	ext{All}(S'')A''} \qquad rac{dash S \, S' \, S'' 	ext{ sig} \quad S dash S' <	herefore S'''}{S dash 	ext{Tuple}(S' \, S'') <: 	ext{Tuple}(S''')}$$

$$rac{S dash 	ext{Rec}(X::	ext{TYPE})A 	ext{ type} \quad S dash 	ext{Rec}(Y::	ext{TYPE})B 	ext{ type} \quad S, Y::	ext{TYPE}, X <: Y dash A <: B}{S dash 	ext{Rec}(X::	ext{TYPE})A <: 	ext{Rec}(Y::	ext{TYPE})B}$$
...
- **[Böhm Berarducci 85]** C.Böhm, A.Berarducci: Automatic synthesis of typed $\lambda$-programs on term algebras, *Theoretical Computer Science*, 39, pp. 135-154, 1985.
...
- **[Cardelli Leroy 90]** L.Cardelli, X.Leroy: Abstract types and the dot notation, *Proceedings of the IFIP TC2 Working Conference on Programming Concepts and Methods, Israel, April 90*.
